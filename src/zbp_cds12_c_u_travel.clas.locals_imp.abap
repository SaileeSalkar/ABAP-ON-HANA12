CLASS lhc_ZCDS12_C_U_TRAVEL DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zcds12_c_u_travel RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR zcds12_c_u_travel RESULT result.

    METHODS create FOR MODIFY
      IMPORTING entities FOR CREATE zcds12_c_u_travel.

    METHODS update FOR MODIFY
      IMPORTING entities FOR UPDATE zcds12_c_u_travel.

    METHODS delete FOR MODIFY
      IMPORTING keys FOR DELETE zcds12_c_u_travel.

    METHODS read FOR READ
      IMPORTING keys FOR READ zcds12_c_u_travel RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK zcds12_c_u_travel.
      METHODS map_data IMPORTING im_data   TYPE zcds12_c_u_travel
                     RETURNING VALUE(r_bapi_data) TYPE /dmo/s_travel_in.


ENDCLASS.

CLASS lhc_ZCDS12_C_U_TRAVEL IMPLEMENTATION.
METHOD map_data.
  ...
  ls_travel_in = map_data( im_data = CORRESPONDING #( <fs_travel_create> ) ).
      ...
      IF lt_messages IS INITIAL .
        ...
        travelid = ls_travel_out-travel_id ) INTO TABLE mapped-zcds12_c_u_travel .
      ELSE.
        LOOP AT lt_messages TRANSPORTING NO FIELDS WHERE msgty = 'E' OR msgty = 'A'.
          ls_travel_in = map_data( im_data = CORRESPONDING #( <fs_travel_create> ) ).
          INSERT VALUE #( %cid = <fs_travel_create>-%cid ) INTO TABLE failed-zcds12_c_u_travel .
          RETURN.
        ENDLOOP.
      ENDIF.
    ENDLOOP.
...
ls_travel_x-travel_id = <to_be_updated>-TravelId.
  ...
  EXPORTING
      ...
      LOOP AT  lt_message TRANSPORTING NO FIELDS WHERE  msgty = 'E' OR msgty = 'A'.
        ...
        INTO table failed-zcds12_c_u_travel .
      ENDLOOP.
    ENDLOOP.
  ...
  DATA lt_messages  TYPE /dmo/t_message.
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<to_be_deleted>) .
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_DELETE'
        EXPORTING
          iv_travel_id = <to_be_deleted>-TravelId
        IMPORTING
          et_messages  = lt_messages.
      LOOP AT lt_messages TRANSPORTING NO FIELDS WHERE msgty = 'E' OR msgty = 'A'.
        INSERT VALUE #(  %cid = <to_be_deleted>-%cid_ref travelid = <to_be_deleted>-TravelId ) INTO TABLE failed-zcds12_c_u_travel.
      ENDLOOP.
    ENDLOOP.
...
CLASS lsc_ZCDS12_C_U_TRAVEL DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZCDS12_C_U_TRAVEL IMPLEMENTATION.
